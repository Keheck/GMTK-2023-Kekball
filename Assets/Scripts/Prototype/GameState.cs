...
public class GameState : MonoBehaviour {
    ...
    async void GenerateTasks() {
        ...
        while (true) {
            int taskType = (int)Random.Range(0, 5);
            switch (taskType) {
                case 0:
                    // dont connect too many players
                    if (players.Count + connectingPlayers.Count >= 6) continue;
                ...
                case 1:
                    ...
                    foreach (Task task in tasks) {
                        if (task.targetPlayer == timedOutPlayer) continue;
                    }
                    // found a player to time out, time them out
                    if (timedOutPlayer.timedOut) continue;
                ...
                case 3:
                    ...
                    foreach (Task task in tasks) {
                        if (task.targetPlayer == player) continue;
                    }
                ...
                case 4:
                    ...
                    // abort if we already have a task for this player
                    foreach (Task task in tasks) {
                        if (task.targetPlayer == player2) continue;
                    }
